...
module.exports = {
    ...
    getcommentPost: asyncHandle(async (req, res, next) => {
        const commentPost = await commentPostService.getcommentPostById(req.params.commentPostId);
        if (!commentPost) {
            return next(new ErrorResponse(httpStatus.NOT_FOUND, "Comment Post not found"));
        }
        res.status(httpStatus.OK).json({
            status: "success",
            commentPost,
    ...
    updateCommentPost: asyncHandle(async (req, res) => {
        const commentPost = await commentPostService.updateCommentPostById(req.params.commentId, req.body);
        res.status(httpStatus.OK).json({
            status: "success",
            commentPost,
        });
