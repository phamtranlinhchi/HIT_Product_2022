...
const getCommentPosts = async (commentPostQuery) => {
    const features = new APIFeatures(CommentPost.find(), commentPostQuery).filter().sort().limitFields().paginate();
...
const updateCommentPostById = async (commentPostId, updateBody) => {
    const commentPost = await CommentPost.findByIdAndUpdate(commentPostId, updateBody, {
        new: true,
        runValidators: true,
    });
    if (!commentPost) {
